...
//	private static final float MAX_SCALE        = 64.0f;//最大的放大倍数
	private static final float MAX_SCALE        = 16.0f;//最大的放大倍数
	...
	public boolean dispatchTouchEvent(MotionEvent ev) {
		switch (ev.getAction()) {
			...
			case MotionEvent.ACTION_MOVE :
				float curX = ev.getX();
				float curY = ev.getY();
				float x = Math.abs(curX - mPreX);
				float y = Math.abs(curY - mPreY);
				if((y>5 && mScale > 1)|| x > 20 || ev.getPointerCount() > 1) {
				    getParent().requestDisallowInterceptTouchEvent(true);
				}
				break;
		}
		Log.e("TAG", "mXScroll="+mXScroll +"   mYScroll="+mYScroll);
